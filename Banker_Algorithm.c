...
int main()
{
    ...
    while (k)
    {
        ...
        for (i = 0; i < n; i++) //Process
        {
            ...
            {
                c = 0;
                for (j = 0; j < m; j++) //Resouce
                {
                    if (need[i][j] <= ava[j])
                    {
                        c++; //If this resource type
                    }
                }
                if (c == m) //All resource type can be satisfied
                {
                    array[z++] = i; // Storing the process number , which is being exec
                    printf("Resouces can be allocated to Process:%d and available resources are: ", (i+ 1));

                    for (j = 0; j < m; j++)
                    {
                        printf("%d ", ava[j]); //Print the available before process exec
                    }
                    printf("\n");
                    for (j = 0; j < m; j++)
                    {
                        ava[j] += all[i][j]; //Print the available after process exec
                        all[i][j] = 0;
                    }
                    flag[i] = 0; //Because this process is already executed
                    count++; //Have count of number of process executed
                }
            }
        }
